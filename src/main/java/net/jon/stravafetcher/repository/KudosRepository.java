...
//            "GROUP BY f.id " +
//            "HAVING COALESCE(COUNT(k), 0) = (SELECT MIN(subquery.kudos_count) FROM (SELECT f2.id AS follower_id, COALESCE(COUNT(k2), 0) AS kudos_count FROM Follower f2 LEFT JOIN Kudos k2 ON f2.id = k2.follower.id LEFT JOIN RideActivity a2 ON a2.id = k2.activityId WHERE (k2.activityId IS NULL OR a2.startDateLocal >= :dateFrom) GROUP BY f2.id) AS subquery) " +
